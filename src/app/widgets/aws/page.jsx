...
import AWSWidget from '../../dashboard/components/AWSWidget';
import { Skeleton } from '@progress/kendo-react-indicators';

function AWSSkeleton() {
  return (
...
export default function AWSEmbedPage({ searchParams }) {
    ...
    async function fetchData() {
      try {
        const response = await apiClient.post(
          "https://api-ccrdev.insight.com/ccr-dashboard-service/aws/spend",
          payload
        );
  ...
  return (
    <div className="dashboard-widget" style={{ maxWidth: 420, margin: "32px auto" }}>
      {isLoading ? <AWSSkeleton /> : <AWSWidget data={apiData || {}} />}
    </div>
  );
}
