...
import AdobeWidget from '../../dashboard/components/AdobeWidget';
import { Skeleton } from '@progress/kendo-react-indicators';

function AdobeSkeleton() {
  return (
...
export default function AdobeEmbedPage({ searchParams }) {
    ...
    async function fetchData() {
      try {
        const response = await apiClient.post(
          "https://api-ccrdev.insight.com/ccr-dashboard-service/adobe/spend",
          payload
        );
  ...
  return (
    <div className="dashboard-widget" style={{ maxWidth: 420, margin: "32px auto" }}>
      {isLoading ? <AdobeSkeleton /> : <AdobeWidget data={apiData || {}} />}
    </div>
  );
}
