...
// --- SVG Icons ---
// Using inline SVGs to avoid external dependencies and allow easy styling.
...
const navItems = [
  ...
  { label: "Download Reports", children: [] },
  {
    label: "Admin",
    children: [
      { label: "Laptops" },
...
const Header = () => {
  ...
  // Function to handle clicking on a main navigation item
  const handleMenuClick = (index) => {
    // If the clicked menu is already open, close it. Otherwise, open it.
    setOpenMenuIndex(openMenuIndex === index ? null : index);
  };

  // Effect to handle clicks outside the navigation menu to close any open dropdowns
  useEffect(() => {
          ...
          <div className={styles.logo}>
            ...
            <Link href="/">
              <svg viewBox="0 -20 460 140" className={styles.svg}>
                <title className="c-icon__title">{"title"}</title>
                <g>
                  <g id="Symbol">
                    <path
                      fill="#721357"
              ...
              {navItems.map((item, index) => (
                <li key={index} className={styles.navItem}>
                  <button
                    ...
                    // Disable button default behavior if it's just a link
                    type={item.children.length > 0 ? "button" : "button"}
                    aria-haspopup={item.children.length > 0}
                    aria-expanded={openMenuIndex === index}
                  >
                    {item.label}
                    {/* Show chevron only if there are child items */}
                  ...
                  {item.children.length > 0 && (
                    <ul
                      className={`${styles.dropdown} ${
                        openMenuIndex === index ? styles.dropdownOpen : ""
                      }`}
                    >
